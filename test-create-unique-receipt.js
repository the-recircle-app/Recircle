/**
 * Test script to create a unique receipt for manual approval via Google Sheets
 * This creates a receipt with different values to avoid duplicate detection
 */

import fetch from 'node-fetch';

async function createUniqueReceipt() {
  try {
    console.log('🧪 Creating unique test receipt for Google Sheets manual approval...');
    ...
    const amount = 67.89; // Different amount to avoid duplicate detection
    const receiptPayload = {
      userId: 102,
      storeId: null,
      storeName: "Unique Test Shop",
      amount: amount,
      purchaseDate: new Date().toISOString(),
      receiptId: `UNIQUE-${timestamp}`,
      status: "pending_review",
      confidence: 0.8,
      isSustainable: true,
      requiresManualReview: true,
      reviewReason: "Created for Google Sheets approval test - unique values",
      estimatedReward: 10  // Different reward for testing
    };
...
4. Check that the complete URL is:
   https://recirclerewards.app/api/receipt-approved

5. Watch the app interface
   - The token balance should automatically update from ${userBefore.tokenBalance} to ${userBefore.tokenBalance + 10}
