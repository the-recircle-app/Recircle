...
import { Receipt, ThriftStore, Transaction } from '@shared/schema';
 ...
 * 
 * NEW DISTRIBUTION MODEL (May 2025):
 * The user now receives 70% of the total minted tokens, while 30% is split
 * between the creator fund and app fund. This replaces the old model where
 * users got 100% and an additional 100% was minted for sustainability.
 */
export const ECOSYSTEM_MULTIPLIERS = {
  // User gets 70% of total rewards
  USER_MULTIPLIER: 0.7,
  
  // Creator fund gets 15% of total rewards (half of the 30% sustainability portion)
  CREATOR_MULTIPLIER: 0.15,
  
  // App fund gets 15% of total rewards (half of the 30% sustainability portion)
  APP_MULTIPLIER: 0.15,
  
  // Internal reference: combined multiplier is 100% (USER + CREATOR + APP = 1.0)
};
...
export function calculateSustainabilityRewards(totalRewardAmount: number): {
  ...
  // Calculate creator reward (15% of total reward)
  const creatorReward = Math.round(totalRewardAmount * ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 10) / 10;
  
  // Calculate app reward (15% of total reward)
  const appReward = Math.round(totalRewardAmount * ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 10) / 10;
  
  // Total sustainability reward (30% of total)
  const totalSustainabilityReward = creatorReward + appReward;
  ...
  console.log(`Reward distribution (70/30 split):
    ...
    - Creator fund: ${creatorReward} B3TR (${ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 100}% of total)
    - App fund: ${appReward} B3TR (${ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 100}% of total)
    - Total sustainability: ${totalSustainabilityReward} B3TR (${(ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER + ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER) * 100}% of total)`);
 ...
 * - Creator wallet: 12 x 0.15 = 1.8 tokens (15% of total)
 * - App wallet: 12 x 0.15 = 1.8 tokens (15% of total)
 * - Total: 12 tokens (8.4 + 1.8 + 1.8)
 */
export function applyStreakMultiplier(baseAmount: number, streakInfo: StreakInfo): number {
  ...
  console.log(`Note: This raw amount will be distributed according to the 70/30 model (user: ${amount * ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER}, sustainability: ${amount * (ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER + ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER)})`);
 ...
 * - Creator wallet: 10 x 0.15 = 1.5 tokens (15% of total)
 * - App wallet: 10 x 0.15 = 1.5 tokens (15% of total)
 * - Total: 10 tokens (7 + 1.5 + 1.5)
 */
