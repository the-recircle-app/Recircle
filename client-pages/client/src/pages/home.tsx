...
const Home = () => {
  ...
  // Get user data including streak info with frequent refreshing
  ...
  // Force refetch user data when the component mounts or when receipts change
  useEffect(() => {
    if (userId && isConnected) {
      // Refresh token balance directly from the server
      if (refreshTokenBalance) {
        refreshTokenBalance().then(newBalance => {
          console.log("Home page refreshed token balance:", newBalance);
        });
      }
  ...
  }, [userId, isConnected, receipts, refetchUserData, refreshTokenBalance]);
  
  // Add event listener for receipt approvals
  useEffect(() => {
    // Function to handle receipt approval events
    const handleReceiptApproval = () => {
      console.log("Receipt approval event detected, refreshing data...");
      if (refreshTokenBalance) {
        refreshTokenBalance().then(newBalance => {
          console.log("Token balance updated after receipt approval:", newBalance);
        });
      }
      refetchUserData();
    };
  ...
  useEffect(() => {
    if (isConnected) {
      ...
      console.log("Home page stats update:", {
        userId,
        isConnected,
  ...
  }, [isConnected, tokenBalance, receipts, transactions, userData, stats.totalRewards, stats.receiptsCount, stats.streak, stats.dailyActions]);

  const blue = "#38BDF8"; // Bright cyan/blue for dark theme
  
  return (
    <div className="pb-20"> {/* Add padding bottom for the navigation bar */}
      <Header gems={tokenBalance} streak={stats.streak} className="mb-2" />
      
      {/* Auto-refresh token balance component */}
      {isConnected && userId && (
      ...
      <div className="p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="font-bold text-xl text-gray-100">Ride | Share | Reward</h2>
            ...
            <div className="bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-700 mb-4">
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-400">Scan secondhand purchase receipts or add re-use stores to unlock achievements</p>
                </div>
              </div>
            </div>
          )
          ...
          <div className="bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-700 text-center">
            <div className="flex flex-col items-center justify-center py-4">
              ...
              <p className="text-sm text-gray-400 mb-4">
                Connect your wallet to earn B3TR tokens for shopping secondhand and giving items a second life
              </p>
              <Link href="/connect-wallet?returnTo=/" className="block w-4/5 mx-auto">
                <button className="w-full py-2.5 px-4 bg-blue-600 text-white rounded-full font-medium hover:bg-blue-500 transition-colors">
                  Connect Wallet
                </button>
              </Link>
            </div>
