...
export const REWARD_TIERS: { [key: string]: RewardTier } = {
  ...
  thrift_store: { min: 2, max: 5, category: "Thrift Store Purchase", priority: 2 },
...
export function calculateReceiptReward(receiptData: any): number {
  ...
  if (category) {
    ...
    else if (categoryLower.includes('thrift') || categoryLower.includes('goodwill') || categoryLower.includes('salvation')) {
      tier = REWARD_TIERS.thrift_store;
...
export function calculateEnvironmentalImpact(receiptData: any): { metric: string; value: number; unit: string } {
  ...
  if (categoryLower.includes('thrift') || categoryLower.includes('pre_owned') || categoryLower.includes('used')) {
...
export function getRewardTierInfo(category: string): RewardTier {
  ...
  if (categoryLower.includes('thrift')) {
    return REWARD_TIERS.thrift_store;
  }
