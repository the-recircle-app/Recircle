...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/stores", async (req: Request, res: Response) => {
    try {
      ...
      if (storeData.addedBy !== undefined && storeData.addedBy !== null) {
        ...
        // Create transaction for creator sustainability reward (15% of total)
        await storage.createTransaction({
          userId: null, // No specific user - this goes to creator wallet
          type: "sustainability_creator",
          amount: distributionRewards.creatorReward,
          description: `Creator Sustainability (Store Addition): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
          referenceId: newStore.id,
          txHash: `txhash-sc-${Date.now().toString(16)}` // Mock hash with unique identifier
        });
        
        // Create transaction for app ecosystem sustainability reward (15% of total)
        await storage.createTransaction({
        ...
        console.log(`Store addition rewards with 70/30 distribution:
          ...
          - Creator fund: ${distributionRewards.creatorReward} B3TR (${ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 100}% of total)
          - App fund: ${distributionRewards.appReward} B3TR (${ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 100}% of total)
          - Total sustainability: ${distributionRewards.totalSustainabilityReward} B3TR (${(ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER + ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER) * 100}% of total)
        `);
        ...
        if (userStores.length === 1 && !wasAchievementAwarded(userId, 'first_store')) {
          ...
          await storage.createTransaction({
            userId: null, // No specific user - this goes to creator wallet
            type: "sustainability_creator",
            amount: achievementDistribution.creatorReward,
            description: `Creator Sustainability (first_store): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
            referenceId: null,
            txHash: `txhash-sca-${Date.now().toString(16)}` // Mock hash with unique identifier
          });
          ...
          console.log(`Achievement first_store rewards with 70/30 distribution:
            ...
            - User portion: ${achievementUserReward} B3TR (${ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER * 100}% of total)
            - Creator fund: ${achievementDistribution.creatorReward} B3TR (${ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 100}% of total)
            - App fund: ${achievementDistribution.appReward} B3TR (${ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 100}% of total)
            - Total sustainability: ${achievementDistribution.totalSustainabilityReward} B3TR (${(ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER + ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER) * 100}% of total)
          `);
        ...
        const responseData = {
          ...
          rewardDetails: {
            ...
            userMultiplier: ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER,
  ...
  app.post("/api/receipts", receiptSubmissionRateLimit, async (req: Request, res: Response) => {
    try {
      ...
      // Always check daily limit, but only enforce it in normal mode (not test mode)
      if (userId) {
        ...
        if (limitReached) {
          ...
          if (!isTestMode) {
            ...
            console.log(`TEST MODE: Allowing action despite daily limit being reached`);
          }
        }
      }
      ...
      try {
        ...
        if (potentialDuplicates.length > 0) {
          ...
          return res.status(400).json({
            message: "Failed to submit receipt due to duplicate",
      ...
      const storeNameProvided = analysisResult.storeName || null;
      ...
      try {
        ...
        switch(receiptData.category) {
          case "thrift_clothing":
            categoryLabel = "Re-Use Item"; // Use consistent display name
            break;
          case "used_video_games":
          case "used_video_games_electronics": // Add legacy category
            categoryLabel = "Re-Use Item"; // Use consistent display name
            break;
          case "used_books":
          case "re-use item": // Add standardized category
            categoryLabel = "Re-Use Item"; // Use consistent display name
            break;
          ...
          case "used_movies":
            ...
            break;
          default:
            categoryLabel = "Re-Use Item"; // Use consistent display name for all categories
        }
  ...
  app.post("/api/achievements/:userId/:type", async (req: Request, res: Response) => {
    try {
      ...
      await storage.createTransaction({
        ...
        type: "sustainability_creator",
        amount: distributionRewards.creatorReward,
        description: `Creator Sustainability (${type}): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
        referenceId: null,
        txHash: `txhash-sca-${Date.now().toString(16)}` // Mock hash with unique identifier
      });
  ...
  app.post("/api/debug/reward-test", async (req: Request, res: Response) => {
    try {
      ...
      // Send the on-chain reward with 70/30 distribution
      ...
      if (txResult.success) {
        ...
        const transaction = await storage.createTransaction({
          userId: userId,
          type: "test_reward",
          amount: reward_amount * ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER, // User gets 70%
          description: `Blockchain test reward (${proof_type || 'debug'})`,
          referenceId: null,
          txHash: txResult.hash
        });
        
        // Create corresponding sustainability transactions for completeness
        await storage.createTransaction({
          userId: null,
          type: "sustainability_creator",
          amount: reward_amount * ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER, // Creator gets 15%
          description: `Creator Sustainability (Test): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
          referenceId: null,
          txHash: txResult.creatorHash || `test-tx-creator-${Date.now()}`
        });
        
        await storage.createTransaction({
          userId: null,
          type: "sustainability_app",
          amount: reward_amount * ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER, // App gets 15%
          description: `App Sustainability (Test): ${APP_FUND_WALLET.slice(0, 8)}...`,
          referenceId: null,
          txHash: txResult.appHash || `test-tx-app-${Date.now()}`
        });
        
        // Update user's token balance with the 70% portion
        const userReward = reward_amount * ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER;
        ...
        return res.json({
          ...
          transactions: {
            ...
            creator: txResult.transactions?.creatorTx || txResult.creatorHash,
          ...
          explorerUrls: {
            ...
            creator: txResult.transactions?.creatorTx ? `https://explore-testnet.vechain.org/transactions/${txResult.transactions.creatorTx}` : null,
  ...
  app.post("/api/token-redemption", async (req: Request, res: Response) => {
    try {
      ...
      // In a production app, we would execute actual blockchain transactions here
      // For example:
      // 1. For claiming tokens: Mint B3TR tokens (VIP-180) directly to the user's connected wallet
      //    - This would involve a contract call to the B3TR token contract's mint function
      //    - The contract would need to implement proper access controls to allow only this app to mint tokens
      //    - The connected wallet address would be used as the destination for the tokens
      //
      // 2. For wallet transfers: Create a VIP-180 token transfer transaction
      //    - This could either be a direct transfer from the app's wallet (if tokens are pre-minted)
      //    - Or it could mint new tokens directly to the destination address
      //    - In either case, the transaction would be signed by the app's private key and broadcast to the VeChain network
  ...
  app.post("/api/admin/approve-submission/:id", requireAdmin, async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Google Form store submission approved and rewards created:
        ...
        - Creator sustainability: ${sustainabilityRewards.creatorReward} B3TR (${ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 100}%)
        - App sustainability: ${sustainabilityRewards.appReward} B3TR (${ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 100}%)
  ...
  app.post("/api/receipt-approved", async (req: Request, res: Response) => {
    try {
      ...
      console.log(`Rewards for approved receipt (70/30 distribution):
        ...
        - User reward: ${distributionRewards.userReward} B3TR (${ECOSYSTEM_MULTIPLIERS.USER_MULTIPLIER * 100}% of total)
        - Creator fund: ${distributionRewards.creatorReward} B3TR (${ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER * 100}% of total)
        - App fund: ${distributionRewards.appReward} B3TR (${ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER * 100}% of total)
        - Total sustainability portion: ${distributionRewards.totalSustainabilityReward} B3TR (${(ECOSYSTEM_MULTIPLIERS.CREATOR_MULTIPLIER + ECOSYSTEM_MULTIPLIERS.APP_MULTIPLIER) * 100}% of total)
      `);
      ...
      const transaction = await storage.createTransaction({
        ...
        amount: finalReward,
        description: description,
        referenceId: receipt_id ? Number(receipt_id) : null,
        txHash: txHash 
      });
      ...
      await storage.createTransaction({
        ...
        type: "sustainability_creator",
        amount: distributionRewards.creatorReward,
        description: `Creator Sustainability (Manual Review): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
        referenceId: receipt_id ? Number(receipt_id) : null,
        txHash: `txhash-mrc-${Date.now().toString(16)}` // Mock hash with unique identifier
      });
      ...
      if (!await wasAchievementAwarded(userId, 'first_receipt')) {
        ...
        // Create sustainability transactions for achievement reward
        await storage.createTransaction({
          userId: null, // Creator wallet
          type: "sustainability_creator",
          amount: achievementDistribution.creatorReward,
          description: `Creator Sustainability (Achievement): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
          referenceId: receipt_id ? Number(receipt_id) : null,
          txHash: `txhash-ach-cr-${Date.now().toString(16)}`
        });
    ...
    try {
      ...
      if (user_wallet && user_wallet.startsWith('0x')) {
        try {
          ...
          // Send the on-chain reward with blockchain distribution
          const txResult = await sendReward({
            recipient: user_wallet,
            amount: amountInWei,
            proofTypes: proofData.proofTypes,
          ...
          if (txResult && txResult.success) {
            // Use the actual transaction hash from the blockchain
            blockchainTxHash = txResult.hash || blockchainTxHash;
      ...
      await storage.createTransaction({
        ...
        type: "sustainability_creator",
        amount: distributionRewards.creatorReward,
        description: `Creator Sustainability (Google Sheet Store): ${CREATOR_FUND_WALLET.slice(0, 8)}...`,
        referenceId: null,
        txHash: `txhash-gsc-${Date.now().toString(16)}` // Mock hash with unique identifier
      });
      ...
      await storage.createTransaction({
        ...
        type: "sustainability_app",
        amount: distributionRewards.appReward,
        description: `App Sustainability (Google Sheet Store): ${APP_FUND_WALLET.slice(0, 8)}...`,
        referenceId: null,
        txHash: `txhash-gsa-${Date.now().toString(16)}` // Mock hash with unique identifier
      });
