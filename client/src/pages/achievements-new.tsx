...
const AchievementsPage: React.FC = () => {
  ...
  const { data: userData } = useQuery<User>({
    ...
    enabled: true, // Always enabled for test user
  });
  
  // Fetch receipts with proper typing
  const { data: receipts = [] } = useQuery<Receipt[]>({
    queryKey: [`/api/users/${currentUserId}/receipts`],
    enabled: true, // Always enabled for test user
  });
  ...
  const categories = [
    ...
    { id: 'shopping', label: 'Shopping', icon: <Leaf className="h-4 w-4" /> },
  ...
  const allAchievements = [
    { type: 'first_receipt', category: 'shopping' },
    { type: 'five_receipts', category: 'shopping' },
    { type: 'ten_receipts', category: 'shopping' },
